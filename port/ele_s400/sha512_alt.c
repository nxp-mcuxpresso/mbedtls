                               ...
                               size_t ilen )
{
    ...
    if (ELE_Hash_Update(S3MU, &ctx->ele_ctx, ctx->is384 ? kELE_Sha384 : kELE_Sha512, (const uint8_t *)input, ilen) != kStatus_Success)
    {
        ret = MBEDTLS_ERR_SHA512_HW_ACCEL_FAILED;
    }
    else
                               ...
                               unsigned char output[64] )
{
    ...
    if (ELE_Hash_Finish(S3MU, &ctx->ele_ctx, ctx->is384 ? kELE_Sha384 : kELE_Sha512, output, NULL, 0u) != kStatus_Success)
    {
        ret = MBEDTLS_ERR_SHA512_HW_ACCEL_FAILED;
    }
    else
