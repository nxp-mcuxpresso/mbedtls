                         ...
                         unsigned char *output )
{
    ...
    else if ((sss_sscp_key_store_set_key(&g_keyStore, &sssKey, key, (keylen + 7u) / 8u,
                                               keylen, NULL, 0u)) != kStatus_SSS_Success)
    {
    ...
    {
      PRINTF("DBG MSG: sss_sscp_mac_context_init failed. \r\n");
      ret = -4;
    }    
    /* RUN CMAC ONE GO */
    else if (sss_sscp_mac_one_go(&ctx, (const uint8_t *)input, ilen, (uint8_t *)output, &macSize) != kStatus_SSS_Success)
    {
      PRINTF("DBG MSG: sss_sscp_mac_one_go failed. \r\n");
      ret = -5;
    }
    else {
      ret = 0;
    ...
    /* CLEAN UP */
    /* KEYOBJECT FREE*/
    if (sss_sscp_key_object_free(&sssKey) != kStatus_SSS_Success)
    {
    ...
    {
      PRINTF("DBG MSG: sss_sscp_mac_context_free failed.\r\n");
    }
