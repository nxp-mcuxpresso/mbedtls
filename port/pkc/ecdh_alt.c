...
#error \
    The 3 alternative implementations shall be enabled together, and the feature to restart the operation has to be disabled.
...
static void mbedtls_ecp_free_ecdh(mcuxClEcc_DomainParam_t *pDomainParams,
                                ...
                                void *p_rng)
{
    ...
    if (0u !=
        mbedtls_mpi_write_binary(&Q->X, (unsigned char *) PointMultParams.pPoint, pByteLength)) {
        mbedtls_ecp_free_ecdh(&pDomainParams, &PointMultParams);
        return_code = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        goto cleanup;
    }
    if (0u !=
        mbedtls_mpi_write_binary(&Q->Y, (unsigned char *) PointMultParams.pPoint + pByteLength,
                                 pByteLength)) {
        mbedtls_ecp_free_ecdh(&pDomainParams, &PointMultParams);
        return_code = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        goto cleanup;
...
#endif  \
    /* MBEDTLS_ECDH_GEN_PUBLIC_ALT && MBEDTLS_ECDH_COMPUTE_SHARED_ALT && MBEDTLS_ECDH_CANDO_ALT && !MBEDTLS_ECP_RESTARTABLE */
