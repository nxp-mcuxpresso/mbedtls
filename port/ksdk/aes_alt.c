...
int mbedtls_aes_setkey_enc(mbedtls_aes_context *ctx, const unsigned char *key, unsigned int keybits)
{
    ...
    HASHCRYPT_AES_SetKey(HASH, ctx, key, key_size);
...
{
    HASHCRYPT_AES_EncryptEcb(HASH, ctx, input, output, 16);
...
{
    HASHCRYPT_AES_DecryptEcb(HASH, ctx, input, output, 16);
                          ...
                          unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT)
    {
        ...
        HASHCRYPT_AES_DecryptCbc(HASH, ctx, input, output, length, iv);
        memcpy(iv, tmp, 16);
    }
    else
    {
        HASHCRYPT_AES_EncryptCbc(HASH, ctx, input, output, length, iv);
...
int mbedtls_aes_crypt_ctr(mbedtls_aes_context *ctx,
                                      ...
                                      unsigned char *output)
{
    HASHCRYPT_AES_CryptCtr(HASH, ctx, input, output, length, nonce_counter, stream_block,
                                    nc_off);
