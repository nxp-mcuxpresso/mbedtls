...
static int wm_wrap_entropy_poll(void *data,
...
{
		int ret = 0;
		const unsigned char *custom_name =
			(const unsigned char *) "WMSDK_4.0";
		size_t custom_name_len = strlen((const char *) custom_name);

		mbedtls_entropy_init(&wm_mbedtls_gdata.entropy);

		mbedtls_entropy_add_source(&wm_mbedtls_gdata.entropy,
				wm_wrap_entropy_poll,
				NULL,
		...
		if ((ret = mbedtls_ctr_drbg_seed(&wm_mbedtls_gdata.ctr_drbg,
						mbedtls_entropy_func,
						&wm_mbedtls_gdata.entropy,
						custom_name,
						custom_name_len)) != 0) {
			wm_mbedtls_e("mbedtls_ctr_drbg_seed returned, "\
					"ret = -0x%02X", -ret);
			return -1;
		}
		return 0;
