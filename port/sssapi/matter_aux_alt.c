                        ...
                        void *p_rng )
{
    ...
    else if (SSS_KEY_STORE_GET_PUBKEY(&ctx->key, pubKey, &keySize, &keyBitsLen) != kStatus_SSS_Success)
    {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    }
    else 
...
int mbedtls_pk_write_key_der( mbedtls_pk_context *key, unsigned char *buf, size_t size )
{
    ...
    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_ECKEY )
    {
        ...
        if ((keyCtx->d.s != MBEDTLS_MPI_S_HAVE_OBJECT) || (keyCtx->d.n != MBEDTLS_MPI_N_HAVE_OBJECT))
        {
                  ...
                  const unsigned char *pwd, size_t pwdlen )
{
   ...
   MBEDTLS_INTERNAL_VALIDATE_RET((key != NULL), MBEDTLS_ERR_ECP_BAD_INPUT_DATA);
    ...
    else if (sss_sscp_key_store_import_key(&g_keyStore, &keyCtx->key, key, keylen, keyCtx->grp.pbits,
                                            kSSS_blobType_ELKE_blob) != kStatus_SSS_Success)
    {
        (void)SSS_KEY_OBJ_FREE(&keyCtx->key);
        return MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
    ...
    {
        if (SSS_KEY_STORE_GET_PUBKEY(&keyCtx->key, pubKey,&keySize, &keyBitsLen) != kStatus_SSS_Success)
        {
            ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
        }
        else
