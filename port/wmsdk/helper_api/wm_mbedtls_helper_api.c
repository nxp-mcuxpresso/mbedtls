		...
		size_t cert_buf_len)
{
	...
	if (ret < 0) {
		wm_mbedtls_e("mbedtls_x509_crt_parse failed, "
				"ret = -0x%02X", -ret);
		os_mem_free(cert);
		return NULL;
	}
		...
		size_t key_len, const unsigned char *pwd, size_t pwd_len)
{
	...
	if (ret != 0) {
		wm_mbedtls_e("mbedtls_pk_parse_key failed, "
				"ret = -0x%02X", -ret);
		os_mem_free(key);
		return NULL;
	}
...
static void wm_mbedtls_f_dbg(void *ctx, int level,
		const char *file, int line,
		const char *str)
{
	wm_mbedtls_d("%s:%04d: |%d| %s", get_basename((char *) file),
			line, level, str);
}
		...
		int endpoint, int authmode)
{
	...
	if (MBEDTLS_SSL_MAX_CONTENT_LEN == 16384 ||
				MBEDTLS_SSL_MAX_CONTENT_LEN < 512)
		mbedtls_ssl_conf_max_frag_len(conf,
	...
	mbedtls_ssl_conf_cert_profile(conf,
			&wm_mbedtls_x509_crt_profile_default);
...
int wm_mbedtls_ssl_conf_buffsizes(mbedtls_ssl_config *conf, size_t in_buf_len,
		size_t out_buf_len)
{
...
int wm_mbedtls_ssl_connect(mbedtls_ssl_context *ssl)
{
	...
	while ((ret = mbedtls_ssl_handshake(ssl)) != 0) {
		if ((ret != MBEDTLS_ERR_SSL_WANT_READ)
			...
			wm_mbedtls_e("mbedtls_ssl_handshake failed, "
					"ret = -0x%02X", -ret);
			break;
		}
	}
	...
	if ((flags = mbedtls_ssl_get_verify_result(ssl)) != 0) {
		...
		mbedtls_x509_crt_verify_info(vrfy_buf,
				sizeof(vrfy_buf), "==> ", flags);

		wm_mbedtls_e("--------------------------------------");
		wm_mbedtls_e("Verification results for flags 0x%02X "
				"are:", flags);
		...
		int fd, const char *hostname)
{
	...
	if (!conf) {
		wm_mbedtls_e("Cannot proceed to setup with "
	...
	if (ret != 0) {
		wm_mbedtls_e("mbedtls_ssl_setup failed, "
				"ret = -0x%02X", -ret);
		return NULL;
	}
	...
	timer = (mbedtls_timing_delay_context *)
		os_mem_alloc(sizeof(mbedtls_timing_delay_context));
	if (!timer)
		return NULL;
...
int wm_mbedtls_ssl_read(mbedtls_ssl_context *ssl, unsigned char *buf,
		size_t len)
{
	int ret = 0;
	...
	if (ret < 0) {
		wm_mbedtls_d("mbedtls_ssl_read failed, ret = -0x%02X",
			-ret);
	}
...
int wm_mbedtls_ssl_write(mbedtls_ssl_context *ssl, const unsigned char *buf,
		size_t len)
{
	int ret = 0, written;
	...
	for (written = 0; written < len; written += ret) {
		do ret = mbedtls_ssl_write(ssl, buf + written, len - written);
		while (ret == MBEDTLS_ERR_SSL_WANT_READ ||
			ret == MBEDTLS_ERR_SSL_WANT_WRITE);

		if (ret <= 0) {
			wm_mbedtls_d("mbedtls_ssl_write failed, ret = -0x%02X",
				-ret);
			return ret;
		}
	}
...
void wm_mbedtls_set_read_timeout(mbedtls_ssl_context *ssl, uint32_t timeout)
{
	...
	mbedtls_ssl_conf_read_timeout(
			(struct mbedtls_ssl_config *) ssl->conf,
			timeout);
}
